...
# Apart from it in case of debian and debug image it also create a testipam-builder image which is based on debian
...
if $CLEAN_BUILD; then
  ...
  if [[ $BASE_OS != "rhel" ]]; then
    docker rmi testipam-builder || true
  fi
  if [[ $DEBUG == 0 ]]; then
...
if [[ $BASE_OS == "rhel" ]]; then
  ...
  -t $IMG_TAG \
  -f $WKDIR/testipam/build-tools/Dockerfile-rhel-multistage \
  ...
  -t testipam-builder \
  -f $WKDIR/testipam/build-tools/Dockerfile-debian-builder \
  --build-arg COVERALLS_TOKEN=${COVERALLS_TOKEN:-false} \
  --build-arg RUN_TESTS=${RUN_TESTS:-false} \
  --build-arg BUILD_VERSION=${BUILD_VERSION} \
  --build-arg BUILD_INFO=${BUILD_INFO} \
  ...
  if [ $DEBUG == 0 ]; then
    docker build --force-rm ${NO_CACHE_ARGS} \
    -t $IMG_TAG-debug \
    -f $WKDIR/testipam/build-tools/Dockerfile-debian-debug \
    --build-arg BUILD_INFO=${BUILD_INFO} \
    --build-arg VERSION_INFO=${VERSION_INFO} \
    --label BUILD_STAMP=$BUILD_STAMP \
    ${VERSION_BUILD_ARGS} \
    ...
    docker build --force-rm ${NO_CACHE_ARGS} \
    -t $IMG_TAG \
    -f $WKDIR/testipam/build-tools/Dockerfile-debian-runtime \
    --build-arg BUILD_INFO=${BUILD_INFO} \
    --build-arg VERSION_INFO=${VERSION_INFO} \
    --label BUILD_STAMP=$BUILD_STAMP \
    ${VERSION_BUILD_ARGS} \
  ...
  if $LICENSE; then
    get_attributions testipam-builder
  fi
fi
